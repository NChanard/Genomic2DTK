    ...
    # Removed Low counts bins
        if(qtlTh.num){
            if(hic.cmx@metadata$symmetric){
                rowBias.num <- Matrix::rowSums(hic.spm,na.rm=TRUE) + Matrix::colSums(hic.spm,na.rm=TRUE) - Matrix::diag(hic.spm)
    ...
    # Bias computation
        ...
        if (hic.cmx@metadata$symmetric){
            if (!is.na(hic.cmx@metadata$kind)){
                rowNormalizer.num <- rowNormalizer.num + Matrix::colSums(hic.spm,na.rm=TRUE) - Matrix::diag(hic.spm)
            }
            colNormalizer.num <- rowNormalizer.num
        }else{
            colNormalizer.num <- Matrix::colSums(hic.spm,na.rm=TRUE)
        }
    hic.cmx@metadata$normalizer = ((rowNormalizer.num[coords.tbl$i] * colNormalizer.num[coords.tbl$j])**(-pow.num)) * sum(hic.spm@x) / sum(hic.spm@x * ((rowNormalizer.num[coords.tbl$i] * colNormalizer.num[coords.tbl$j])**(-pow.num)))
    X.num <- hic.spm@x * (1/(rowNormalizer.num[coords.tbl$i]**pow.num)) * (1/(colNormalizer.num[coords.tbl$j]**pow.num))
