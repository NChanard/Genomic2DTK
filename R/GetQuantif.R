...
GetQuantif <- function(matrices.lst, area.fun = "center", operation.fun = "mean_rm0",
    name.chr = NULL) {
    ...
    if (is.null(operation.fun)) {
        ...
        operation.fun <- dplyr::case_when(operation.fun == "mean_rm0" ~
            "function(x){x[which(x==0)]<-NA;mean(x,na.rm=TRUE)}", operation.fun ==
            "median_rm0" ~ "function(x){x[which(x==0)]<-NA;stats::median(x,na.rm=TRUE)}",
    ...
    if (!is.function(area.fun) && attributes(matrices.lst)$referencePoint ==
        "pf") {
        # Compute rows and cols index Center
        center.num <- c(floor((matriceDim.num + 1)/2) + ifelse(matriceDim.num >=
            9, yes = 1, no = 0), ceiling((matriceDim.num + 1)/2) - ifelse(matriceDim.num >=
            9, yes = 1, no = 0))
        centerStart.num <- min(center.num)
        centerEnd.num <- max(center.num)
        center.chr <- ifelse(centerStart.num == centerEnd.num, yes = centerStart.num,
        ...
        # Coords
        donutCoord.dtf <- rbind(expand.grid(topDonut.num, leftDonut.num),
            expand.grid(topDonut.num, widthDonut.num), expand.grid(topDonut.num,
                rightDonut.num), expand.grid(heigthDonut.num, leftDonut.num),
            expand.grid(heigthDonut.num, rightDonut.num), expand.grid(botDonut.num,
                leftDonut.num), expand.grid(botDonut.num, widthDonut.num),
        ...
        area.fun <- dplyr::case_when(toupper(area.fun) %in% c("C", "CENTER") ~
            list(center.chr, center.chr), toupper(area.fun) %in% c("UL",
            "UPPER_LEFT") ~ list(first.chr, first.chr), toupper(area.fun) %in%
            c("UR", "UPPER_RIGHT") ~ list(first.chr, second.chr), toupper(area.fun) %in%
            c("BL", "BOTTOM_LEFT") ~ list(second.chr, first.chr), toupper(area.fun) %in%
            c("BR", "BOTTOM_RIGHT") ~ list(second.chr, second.chr), toupper(area.fun) %in%
            c("U", "UPPER") ~ list(first.chr, center.chr), toupper(area.fun) %in%
            c("B", "BOTTOM") ~ list(second.chr, center.chr), toupper(area.fun) %in%
            c("L", "LEFT") ~ list(center.chr, first.chr), toupper(area.fun) %in%
            c("R", "RIGHT") ~ list(center.chr, second.chr), toupper(area.fun) %in%
            c("D", "DONUT") ~ list(donut.chr), TRUE ~ list(center.chr,
            center.chr)) |>
            paste(collapse = ",")
        area.fun <- WrapFunction(paste0("function(matrice.mtx){ matrice.mtx[",
            area.fun, "] }"))
    } else if (!is.function(area.fun) && attributes(matrices.lst)$referencePoint ==
        "rf") {
        shiftFactor <- attributes(matrices.lst)$shiftFactor
        # Compute rows and cols index Anchor
        anchorStart.num <- max(1, floor((matriceDim.num - 2) * shiftFactor/(1 +
            2 * shiftFactor)) + 1 - ifelse(matriceDim.num >= 9, yes = 1,
            no = 0))
        anchorEnd.num <- max(1, min(matriceDim.num, floor((matriceDim.num -
            2) * shiftFactor/(1 + 2 * shiftFactor)) + 1 + ifelse(matriceDim.num >=
            9, yes = 1, no = 0)))
        anchor.chr <- ifelse(anchorStart.num == anchorEnd.num, yes = anchorStart.num,
            no = paste0(anchorStart.num, ":", anchorEnd.num))
        # Bait
        baitStart.num <- max(1, ceiling((matriceDim.num - 2) * (1 + shiftFactor)/(1 +
            2 * shiftFactor)) + 2 - ifelse(matriceDim.num >= 9, yes = 1,
            no = 0))
        baitEnd.num <- max(1, min(matriceDim.num, ceiling((matriceDim.num -
            2) * (1 + shiftFactor)/(1 + 2 * shiftFactor)) + 2 + ifelse(matriceDim.num >=
            9, yes = 1, no = 0)))
        bait.chr <- ifelse(baitStart.num == baitEnd.num, yes = baitStart.num,
        ...
        BL.lgk <- (((matriceDim.num + 1)/2) - anchorEnd.num) >= 2 && (baitStart.num -
            ((matriceDim.num + 1)/2)) >= 2
        D.lgk <- sum(U.lgk, R.lgk, B.lgk, L.lgk, UL.lgk, UR.lgk, BR.lgk,
            BL.lgk) >= 1
        ...
        if (D.lgk) {
            ...
            donutCoord.dtf <- rbind(expand.grid(topDonut.num, leftDonut.num),
                ...
                expand.grid(heigthDonut.num, rightDonut.num), expand.grid(botDonut.num,
                  leftDonut.num), expand.grid(botDonut.num, widthDonut.num),
                expand.grid(botDonut.num, rightDonut.num))
            donutCoord.dtf <- dplyr::filter(donutCoord.dtf, donutCoord.dtf$Var1 >=
                1 & donutCoord.dtf$Var2 <= matriceDim.num & donutCoord.dtf$Var1 <=
                donutCoord.dtf$Var2)
            donutRows.chr <- paste(donutCoord.dtf[, 1], collapse = ",")
            donutCols.chr <- paste(donutCoord.dtf[, 2], collapse = ",")
            donut.chr <- paste0("cbind(  c(", donutRows.chr, ")  ,  c(",
                donutCols.chr, ")  )")
        }
        # Wrap index in a function
        area.fun <- dplyr::case_when(toupper(area.fun) %in% c("C", "CENTER") ~
            list(anchor.chr, bait.chr), toupper(area.fun) %in% c("UL",
            "UPPER_LEFT") && UL.lgk ~ list(first.chr, ULwidth.chr), toupper(area.fun) %in%
            c("UR", "UPPER_RIGHT") && UR.lgk ~ list(first.chr, second.chr),
            toupper(area.fun) %in% c("BL", "BOTTOM_LEFT") && BL.lgk ~ list(inner.chr,
                inner.chr), toupper(area.fun) %in% c("BR", "BOTTOM_RIGHT") &&
                BR.lgk ~ list(BRheight.chr, second.chr), toupper(area.fun) %in%
                c("U", "UPPER") && U.lgk ~ list(first.chr, bait.chr), toupper(area.fun) %in%
                c("B", "BOTTOM") && B.lgk ~ list(second.chr, bait.chr),
            toupper(area.fun) %in% c("L", "LEFT") && L.lgk ~ list(anchor.chr,
                first.chr), toupper(area.fun) %in% c("R", "RIGHT") && R.lgk ~
                list(anchor.chr, second.chr), toupper(area.fun) %in% c("D",
                "DONUT") && D.lgk ~ list(donut.chr), TRUE ~ list(anchor.chr,
                bait.chr)) |>
            paste(collapse = ",")
        area.fun <- WrapFunction(paste0("function(matrice.mtx){ matrice.mtx[",
            area.fun, "] }"))
    ...
    if (!is.null(name.chr)) {
        ...
        names.chr_lst <- dplyr::arrange(interactions.dtf, factor(interactions.dtf$submatrix.name,
    ...
    repeted.ndx <- rep(names(lengths.num), lengths.num) |>
        as.numeric()
    # Add attributes
    quantif.num <- unlist(quantif.num[repeted.ndx]) |>
        stats::setNames(unlist(names.chr_lst)) |>
        AddAttr(c(attributes(matrices.lst), interactions = attributes(matrices.lst)$interactions[repeted.ndx],
