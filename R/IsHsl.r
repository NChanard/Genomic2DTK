...
IsHsl <- function(color.col=NULL){
    logical.bln <- lapply(color.col[-1], function(value.num){0<=value.num & value.num<=1}) |> unlist() |> sort()
    logical.bln <- logical.bln[[1]]
    return((class(color.col) %in% c("list", "numeric","integer") && 0<=color.col[[1]] && color.col[[1]]<360 && logical.bln))
    }
