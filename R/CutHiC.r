...
CutHiC <- function(megaHic.cmx, verbose.bln = TRUE) {
    ...
    hic.lst_cmx <- BiocParallel::bplapply(BPPARAM = BiocParallel::SerialParam(progressbar = verbose.bln),
        seq_along(chromComb.lst), function(ele.ndx) {
            # Chromosomes
            ...
            col.regions <- binnedGenome.grn[which(as.vector(binnedGenome.grn@seqnames) ==
                ele.lst[[2]])]
            # Extract matrix
            hic.spm <- megaHic.cmx@matrix[GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$row,
                row.regions)@from, GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$column,
                col.regions)@from]
            ...
            if (!is.null(megaHic.cmx@metadata$removedCounts)) {
                removedCounts <- list(removedCounts = megaHic.cmx@metadata$removedCounts[GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$row,
                  row.regions)@from, GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$column,
                  col.regions)@from])
            }
            # observed
            ...
            if (!is.null(megaHic.cmx@metadata$observed)) {
                ...
                observed <- list(observed = observed.spm[GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$row,
                  row.regions)@from, GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$column,
                  col.regions)@from]@x)
            }
            # normalizer
            ...
            if (!is.null(megaHic.cmx@metadata$normalizer)) {
                ...
                normalizer <- list(normalizer = normalizer.spm[GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$row,
                  row.regions)@from, GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$column,
                  col.regions)@from]@x)
            }
            # expected
            ...
            if (!is.null(megaHic.cmx@metadata$expected)) {
                ...
                expected <- list(expected = expected.spm[GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$row,
                  row.regions)@from, GenomicRanges::findOverlaps(InteractionSet::anchors(megaHic.cmx)$column,
                  col.regions)@from]@x)
            }
            # Metadata
            hic.cmx@metadata <- dplyr::filter(attributes.tbl, attributes.tbl$name ==
                chromComb.lst[[ele.ndx]]) |>
    ...
    hic.lst_cmx <- hic.lst_cmx |>
        ...
        AddAttr(list(resolution = res.num, mtx = mtx.chr, chromSize = tibble::as_tibble(chromSize.dtf),
