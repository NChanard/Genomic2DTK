...
GnpToCm <- function(hic.gnp=NULL, res.num=NULL, chromSize.dtf=NULL, verbose.bln=TRUE, cores.num=1){
    ...
    # Chromosomes Size
        chromSize.tbl <- chromSize.dtf %>% magrittr::set_colnames(.,c("seqnames","seqlengths")) %>% tibble::tibble(.)
    ...
    # Combinaison present in bedpe
        chromosomesCombinaison.chr <- paste0(hic.gnp@first@seqnames, "_", hic.gnp@second@seqnames) %>% unique
        matrixSymmetric.bln <- chromosomesCombinaison.chr %>% stringr::str_split(.,"_") %>% lapply(., function(name.chr){name.chr[[1]]  == name.chr[[2]]}) %>% unlist
    ...
    # Convert
        ...
        if(cores.num==1){
            ...
            hic.cmx_lst <- lapply(seq_len(jobLenght.num),function(combin.ndx){
                ...
                combin.lst <- chromosomesCombinaison.chr[[combin.ndx]] %>% stringr::str_split("_") %>% unlist
                start.ndx <- c(1,S4Vectors::runLength(chromosomesCombinaison.rle)) %>% cumsum %>%.[combin.ndx]
                end.ndx <- S4Vectors::runLength(chromosomesCombinaison.rle) %>% cumsum %>% .[combin.ndx]
                i.num_vec <- hic.gnp@first@ranges@start %>% magrittr::extract(start.ndx:end.ndx) %>% magrittr::divide_by(res.num) %>% ceiling %>% as.integer
                j.num_vec <- hic.gnp@second@ranges@start %>% magrittr::extract(start.ndx:end.ndx) %>% magrittr::divide_by(res.num) %>% ceiling %>% as.integer
                x.num_vec <- hic.gnp %>% S4Vectors::mcols(.) %>% magrittr::use_series(score) %>% magrittr::extract(start.ndx:end.ndx)
                dimensionI.num <- chromSize.tbl %>% dplyr::pull(seqnames) %>% magrittr::equals(combin.lst[1]) %>% which %>% (chromSize.tbl %>%  dplyr::pull(dimension))[.]
                dimensionJ.num <- chromSize.tbl %>% dplyr::pull(seqnames) %>% magrittr::equals(combin.lst[2]) %>% which %>% (chromSize.tbl %>%  dplyr::pull(dimension))[.]
                attributes.lst <- attributes.tbl[combin.ndx,] %>% tibble::add_column(resolution = res.num) %>% as.list
        ...
        hic.cmx_lst %>% magrittr::set_names(., chromosomesCombinaison.chr) %>%  DevTK::AddAttr(.,list(resolution=res.num, chromSize = chromSize.tbl, matricesKind=attributes.tbl )) %>% return(.)
}