...
ExtractSubmatrix <- function(feature.gn = NULL, hic.cmx_lst = NULL, referencePoint.chr = "pf",
    ...
    .GInteractionFormatting <- function(feature.gn, res.num) {
        if (inherits(feature.gn, "GRanges")) {
            feature.gni <- InteractionSet::GInteractions(GenomicRanges::resize(feature.gn,
                1, "start"), GenomicRanges::resize(feature.gn, 1, "end"))
        } else if (inherits(feature.gn, "Pairs") && inherits(feature.gn@first,
            "GRanges") && inherits(feature.gn@second, "GRanges")) {
            feature.gni <- InteractionSet::GInteractions(feature.gn@first,
                feature.gn@second)
        ...
        if (is.null(S4Vectors::mcols(feature.gni)$orientation)) {
            S4Vectors::mcols(feature.gni)$orientation <- (feature.gni ==
                InteractionSet::swapAnchors(feature.gni))
        }

        if (is.null(S4Vectors::mcols(feature.gni)$anchor.bin)) {
            S4Vectors::mcols(feature.gni)$anchor.bin <- paste0(GenomeInfoDb::seqnames(InteractionSet::anchors(feature.gni)$first),
                ":", ceiling(InteractionSet::anchors(feature.gni)$first@ranges@start/res.num))

        }

        if (is.null(S4Vectors::mcols(feature.gni)$bait.bin)) {
            S4Vectors::mcols(feature.gni)$bait.bin <- paste0(GenomeInfoDb::seqnames(InteractionSet::anchors(feature.gni)$second),
                ":", ceiling(InteractionSet::anchors(feature.gni)$second@ranges@start/res.num))
        }

        if (is.null(S4Vectors::mcols(feature.gni)$name)) {
            S4Vectors::mcols(feature.gni)$name <- paste0(S4Vectors::mcols(feature.gni)$anchor.bin,
                "_", S4Vectors::mcols(feature.gni)$bait.bin)
        }
        if (is.null(S4Vectors::mcols(feature.gni)$submatrix.name)) {
            S4Vectors::mcols(feature.gni)$submatrix.name <- paste0(S4Vectors::mcols(feature.gni)$anchor.bin,
                "_", S4Vectors::mcols(feature.gni)$bait.bin)
            S4Vectors::mcols(feature.gni)$submatrix.name[!S4Vectors::mcols(feature.gni)$orientation] <- paste0(S4Vectors::mcols(feature.gni)$bait.bin[!S4Vectors::mcols(feature.gni)$orientation],
                "_", S4Vectors::mcols(feature.gni)$anchor.bin[!S4Vectors::mcols(feature.gni)$orientation])
        }
    ...
    if (referencePoint.chr == "rf") {
        cis.lgk <- ReduceRun(GenomeInfoDb::seqnames(InteractionSet::anchors(feature.gn)$first),
        ...
        ranges.dtf <- dplyr::mutate(ranges.dtf, start = pmin(ranges.dtf$first.start,
            ranges.dtf$second.start))
        ranges.dtf <- dplyr::mutate(ranges.dtf, end = pmax(ranges.dtf$first.end,
            ranges.dtf$second.end))
        feature.gnr <- GenomicRanges::GRanges(seqnames = GenomeInfoDb::seqnames(InteractionSet::anchors(feature.gn)$first),
            ranges = IRanges::IRanges(start = ranges.dtf$start, end = ranges.dtf$end),
            seqlengths = GenomeInfoDb::seqlengths(feature.gn), seqinfo = GenomeInfoDb::seqinfo(feature.gn))
        featureResize.gnr <- GenomicRanges::resize(feature.gnr, width = feature.gnr@ranges@width +
            feature.gn$distance * shiftFactor.num * 2, fix = "center")
        featureResize.gni <- InteractionSet::GInteractions(featureResize.gnr,
            featureResize.gnr)
        S4Vectors::mcols(featureResize.gni) <- S4Vectors::mcols(feature.gn)
    } else if (referencePoint.chr == "pf") {
        featureResize.gni <- suppressWarnings(GenomicRanges::resize(feature.gn,
            width = res.num * (matriceDim.num - 1) + 1, fix = "center"))
    }
    # Filt Out Of Bound
    featureFilt.gni <- featureResize.gni[which(1L <= data.frame(InteractionSet::anchors(featureResize.gni)$first@ranges)[,
        "start"] & data.frame(InteractionSet::anchors(featureResize.gni)$first@ranges)[,
        "end"] <= SeqEnds(InteractionSet::anchors(featureResize.gni)$first) &
        1L <= data.frame(InteractionSet::anchors(featureResize.gni)$second)[,
            "start"] & data.frame(InteractionSet::anchors(featureResize.gni)$second)[,
        "end"] <= SeqEnds(InteractionSet::anchors(featureResize.gni)$second))]
    ...
    chromosomesCombinaison.rle <- ReduceRun(GenomeInfoDb::seqnames(InteractionSet::anchors(featureNoDup.gni)$first),
        GenomeInfoDb::seqnames(InteractionSet::anchors(featureNoDup.gni)$second),
        reduceFun.chr = "paste", sep = "_")
    order.num <- unlist(lapply(unique(S4Vectors::runValue(chromosomesCombinaison.rle)),
        function(comb) {
    ...
    present.ndx <- which(as.vector(chromosomesCombinaison.rle) %in% names(matAnchors.gnr_lst) |
        as.vector(chromosomesCombinaison.rle) %in% unlist(lapply(names(matAnchors.gnr_lst),
            function(name.chr) {
                strsplit(name.chr, "_") |>
                  unlist() |>
                  rev() |>
    ...
    # Extraction
    submatrix.spm_lst <- BiocParallel::bplapply(BPPARAM = BiocParallel::SerialParam(progressbar = verbose.bln),
        seq_along(S4Vectors::runValue(chromosomesCombinaison.rle)), function(combinaison.ndx) {
            ...
            if (combinaisonName.chr %in% names(matAnchors.gnr_lst)) {
                ...
                ovl_row <- data.frame(GenomicRanges::findOverlaps(anchors.gnr[combinaisonStart.ndx:combinaisonEnd.ndx],
                  matAnchors.gnr_lst[[mat.ndx]]$row))
                ovl_row <- dplyr::group_by(ovl_row, queryHits = ovl_row$queryHits)
                ovl_row <- tidyr::nest(ovl_row)
                ovl_col <- data.frame(GenomicRanges::findOverlaps(baits.gnr[combinaisonStart.ndx:combinaisonEnd.ndx],
                ...
                combinaisonName.chr |>
                  ...
                  rev() |>
                  paste(collapse = "_")
            } %in% names(matAnchors.gnr_lst)) {
                mat.ndx <- which(names(hic.cmx_lst) == {
                  combinaisonName.chr |>
                    strsplit("_") |>
                    unlist() |>
                    rev() |>
                ...
                ovl_row <- data.frame(GenomicRanges::findOverlaps(baits.gnr[combinaisonStart.ndx:combinaisonEnd.ndx],
                  matAnchors.gnr_lst[[mat.ndx]]$row))
                ovl_row <- dplyr::group_by(ovl_row, queryHits = ovl_row$queryHits)
                ovl_row <- tidyr::nest(ovl_row)
                ovl_col <- data.frame(GenomicRanges::findOverlaps(anchors.gnr[combinaisonStart.ndx:combinaisonEnd.ndx],
                  matAnchors.gnr_lst[[mat.ndx]]$col))
                ovl_col <- dplyr::group_by(ovl_col, queryHits = ovl_col$queryHits)
                ovl_col <- tidyr::nest(ovl_col)
            }
            multicoreParam <- MakeParallelParam(cores.num = cores.num,
                verbose.bln = FALSE)
            tempSubmatrix.spm_lst <- BiocParallel::bplapply(BPPARAM = multicoreParam,
                seq_along(combinaisonStart.ndx:combinaisonEnd.ndx), function(range.ndx) {
                  ...
                  if (dim(mat.spm)[1] != matriceDim.num) {
                    mat.spm <- ResizeMatrix(matrice.mtx = mat.spm, newDim.num = c(matriceDim.num,
                      matriceDim.num))
                  }
                  if (abs(gap.num) < matriceDim.num) {
                    if (abs(gap.num) > 0) {
                      mat.spm <- PadMtx(mat.mtx = mat.spm, padSize.num = abs(gap.num),
                        value.num = 0, side.chr = c("left", "bot"))
                    }
                    mat.spm[lower.tri(mat.spm)] <- NA
                    if (abs(gap.num) > 0) {
                      mat.spm <- mat.spm[seq_len(matriceDim.num), (abs(gap.num) +
                        1):(matriceDim.num + abs(gap.num))]
                    }
                  }
