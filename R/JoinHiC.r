...
JoinHiC <- function (hic.cmx_lst){
    ...
    mapPosition <- data.frame(apply(mapNames |> strsplit("_") |> simplify2array() |> t(), 1:2, function(ele){
        which(chromSize.dtf$name == ele)
    }))
    colnames(mapPosition) <- c("i","j")

    megaDim = sum(chromSize.dtf$dimension)
    mega.dtf <- data.frame(i=NULL,j=NULL,x=NULL)
    for (i in seq_along(chromSize.dtf$name)){
            addI <- 0
            if(i>1){addI <- sum(chromSize.dtf$dimension[1:i-1])}
    ...
    mega.spm = Matrix::sparseMatrix(i=mega.dtf$i, j=mega.dtf$j, x=mega.dtf$x, dims=c(megaDim,megaDim))
    seqlengths.lst <- chromSize.dtf$length|> stats::setNames(chromSize.dtf$name)
    binnedGenome.gnr <- GenomicRanges::tileGenome(seqlengths.lst, tilewidth=attributes(hic.cmx_lst)$resolution, cut.last.tile.in.chrom=TRUE)
    megaHic.cmx <- InteractionSet::ContactMatrix(mega.spm, binnedGenome.gnr, binnedGenome.gnr)
    megaHic.cmx@metadata <- attributes(hic.cmx_lst)[-which(names(attributes(hic.cmx_lst)) =="names")]
