...
SearchPairs <- function(indexAnchor.gnr = NULL, indexBait.gnr = NULL, minDist.num = NULL,
    ...
    pairs.gni_lst <- BiocParallel::bplapply(
        BPPARAM = multicoreParam, seq_along(commonConstraint.lst),
    ...
    names(S4Vectors::mcols(pairs.gni)) <- gsub("anchor1.", "anchor.", gsub(
        "anchor2.",
    ...
    S4Vectors::mcols(pairs.gni)$submatrix.name[!S4Vectors::mcols(pairs.gni)$orientation] <- paste0(
        S4Vectors::mcols(pairs.gni)$bait.bin[!S4Vectors::mcols(pairs.gni)$orientation],
