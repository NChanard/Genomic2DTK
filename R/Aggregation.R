...
Aggregation <- function(ctrlMatrices.lst = NULL, matrices.lst = NULL, minDist.num = NULL,
    ...
    # Aggregate
    agg.mtx <- apply(simplify2array(matrices.lst), seq_len(2), agg.fun)
    gc()
    ...
    if (!is.null(ctrlMatrices.lst)) {
        ...
        ctrlMatrices.lst <- .PrepareMtxList(matrices.lst = ctrlMatrices.lst,
            ...
            trans.fun = trans.fun)
        # Aggregate
        aggCtrl.mtx <- apply(simplify2array(ctrlMatrices.lst), seq_len(2), agg.fun)
        gc()
        ...
        aggDiff.mtx <- apply(simplify2array(diffmatrices.lst), seq_len(2), agg.fun)
        gc()
