...
IndexFeatures <- function(gRange.gnr_lst = NULL, constraint.gnr = NULL,
    chromSize.dtf = NULL, binSize.num = NULL, method.chr = "mean", variablesName.chr_vec = NULL,
    cores.num = 1, verbose.bln = FALSE) {
    # Constraint Informations
    if (is.null(constraint.gnr)) {
        constraint.gnr <- GenomicRanges::GRanges(seqnames = chromSize.dtf[,
            1], ranges = IRanges::IRanges(start = rep(1, length(chromSize.dtf[,
            2])), end = chromSize.dtf[, 2]), strand = "*", name = chromSize.dtf[,
            1])
    } else {
        if (is.null(constraint.gnr$name) | length(which(!is.na(constraint.gnr$name))) ==
            0) {
    ...
    binnedConstraint.gnr <- BinGRanges(gRange.gnr = constraint.gnr, chromSize.dtf = chromSize.dtf,
        binSize.num = binSize.num, verbose.bln = verbose.bln, reduce.bln = FALSE,
        cores.num = cores.num)
    # Feature Names
    ...
    if (is.null(names(gRange.gnr_lst))) {
        gRange.gnr_lst <- stats::setNames(gRange.gnr_lst, paste0("Feature_",
    ...
    # GRanges Binning
    binnedFeature.lst <- BiocParallel::bplapply(BPPARAM = BiocParallel::SerialParam(progressbar = verbose.bln),
        seq_along(gRange.gnr_lst), function(feature.ndx) {
            feature.chr <- feature.chr_vec[[feature.ndx]]
            feature.gnr <- IRanges::subsetByOverlaps(gRange.gnr_lst[[feature.chr]],
                constraint.gnr)
            GenomeInfoDb::seqlevelsStyle(feature.gnr) <- seqLevelsStyle.chr
            binnedFeature.gnr <- BinGRanges(gRange.gnr = feature.gnr, chromSize.dtf = chromSize.dtf,
                binSize.num = binSize.num, method.chr = method.chr, variablesName.chr_vec = variablesName.chr_vec,
                verbose.bln = verbose.bln, reduce.bln = TRUE, cores.num = cores.num)
            binnedFeat.tbl <- tibble::tibble(BinnedFeature.ndx = seq_along(binnedFeature.gnr),
            ...
            binnedConstraint.tbl <- tibble::tibble(BinnedConstraint.ndx = seq_along(binnedConstraint.gnr),
                Constraint.name = binnedConstraint.gnr$name)
            binnedConstraint.tbl <- dplyr::group_by(binnedConstraint.tbl,
            ...
            featConstOvlp.ovlp <- GenomicRanges::findOverlaps(feature.gnr,
                constraint.gnr)
            featConstOvlp.tbl <- tibble::tibble(Feature.name = feature.gnr$name[featConstOvlp.ovlp@from],
            ...
            multicoreParam <- MakeParallelParam(cores.num = cores.num,
                verbose.bln = FALSE)
            binnedFeature.gnr_lst <- BiocParallel::bplapply(BPPARAM = multicoreParam,
                seq_len(nrow(featConstOvlp.tbl)), function(row.ndx) {
                  ranges.ndx <- featConstOvlp.tbl$BinnedFeature.ndx[row.ndx] |>
                    unlist(use.names = FALSE)
                  constraint.ndx <- featConstOvlp.tbl$BinnedConstraint.ndx[row.ndx] |>
                    unlist(use.names = FALSE)
                  subBinnedFeature.gnr <- IRanges::subsetByOverlaps(binnedFeature.gnr[ranges.ndx],
            ...
            binnedFeature.gnr <- MergeGRanges(binnedFeature.gnr_lst, sort.bln = FALSE,
                reduce.bln = FALSE)
            ...
            names(S4Vectors::mcols(binnedFeature.gnr))[which(names(S4Vectors::mcols(binnedFeature.gnr)) ==
                paste0(feature.chr, ".bin"))] <- "bin"
            names(S4Vectors::mcols(binnedFeature.gnr))[which(names(S4Vectors::mcols(binnedFeature.gnr)) ==
                paste0(feature.chr, ".constraint"))] <- "constraint"
    ...
    if (length(idDuplicated.ndx)) {
        ...
        binnedIndexDuplicated.tbl <- dplyr::group_by(binnedIndexDuplicated.tbl,
        ...
        binnedIndexDuplicated.lst <- BiocParallel::bplapply(BPPARAM = multicoreParam,
            seq_len(nrow(binnedIndexDuplicated.tbl)), function(row.ndx) {
        ...
        binnedIndex.gnr <- rbind(binnedIndexDuplicated.tbl, binnedIndexNoDuplicated.tbl) |>
            data.frame() |>
            methods::as("GRanges")
    }
    for (featureName.chr in feature.chr_vec) {
        colname.chr <- paste0(featureName.chr, ".bln")
        S4Vectors::mcols(binnedIndex.gnr)[which(is.na(S4Vectors::mcols(binnedIndex.gnr)[,
            colname.chr])), colname.chr] <- FALSE
        S4Vectors::mcols(binnedIndex.gnr)[, colname.chr] <- methods::as(S4Vectors::mcols(binnedIndex.gnr)[,
            colname.chr], "Rle")
    }
    columOrder.chr <- unique(c("name", "bin", "constraint", names(S4Vectors::mcols(binnedIndex.gnr))))
    S4Vectors::mcols(binnedIndex.gnr) <- as.data.frame(S4Vectors::mcols(binnedIndex.gnr)) |>
