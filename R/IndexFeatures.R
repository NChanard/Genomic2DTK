...
IndexFeatures <- function(gRange.gnr_lst = NULL, constraint.gnr = NULL,
    ...
    if (is.null(constraint.gnr)) {
        constraint.gnr <- GenomicRanges::GRanges(seqnames = chromSize.dtf[
            ,
            1
        ], ranges = IRanges::IRanges(start = rep(1, length(chromSize.dtf[
            ,
            2
        ])), end = chromSize.dtf[, 2]), strand = "*", name = chromSize.dtf[
            ,
            1
        ])
    } else {
    ...
    binnedConstraint.gnr <- BinGRanges(
        gRange.gnr = constraint.gnr, chromSize.dtf = chromSize.dtf,
    ...
    if (is.null(names(gRange.gnr_lst))) {
        gRange.gnr_lst <- stats::setNames(gRange.gnr_lst, paste0(
            "Feature_",
    ...
    binnedFeature.lst <- BiocParallel::bplapply(
        ...
        seq_along(gRange.gnr_lst), function(feature.ndx) {
            ...
            binnedFeature.gnr <- MergeGRanges(binnedFeature.gnr_lst,
                sort.bln = FALSE,
                reduce.bln = FALSE
            )
            ...
            binnedFeature.gnr$name <- paste0(
                binnedFeature.gnr$bin, ":",
