...
ExpectedHiC <- function(hic.cmx_lst, verbose.bln = FALSE, cores.num = 1) {
    ...
    expected.lst <- BiocParallel::bplapply(BPPARAM = multicoreParam, seq_along(matricesNames.chr),
        function(matrixName.ndx) {
            ...
            if (hic.cmx_lst[[matrixName.chr]]@metadata$type == "cis") {
                ...
                hic.dtf <- dplyr::mutate(hic.dtf, distance = 1 + (hic.dtf$j -
                  hic.dtf$i) * resolution.num) |>
                  dplyr::select(c("x", "distance"))
                expected.dtf <- dplyr::group_by(hic.dtf, distance = hic.dtf$distance) |>
                  dplyr::summarise_at(.vars = "x", .funs = list(expected = mean))
                expected.num <- dplyr::left_join(hic.dtf, expected.dtf,
                  by = "distance")
            } else {
                expected.num <- list(expected = sum(hic.cmx_lst[[matrixName.chr]]@matrix@x)/Reduce(`*`,
                  hic.cmx_lst[[matrixName.chr]]@matrix@Dim))
            }
    ...
    expected.dtf <- expected.lst[cisNames.chr] |>
        do.call(what = rbind)
    attributes(hic.cmx_lst)$expected <- dplyr::group_by(expected.dtf, distance = expected.dtf$distance) |>
